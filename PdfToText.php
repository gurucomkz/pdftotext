...
//		'\b ( (BT) | (ET) ) \s+',
    ...
    private function __load($filename, $contents, /** @noinspection PhpUnusedParameterInspection */
	 ...
	 *-------------------------------------------------------------------------------------------------------------*/
    protected function DecodeData($object_id, $stream_data, $type, /** @noinspection PhpUnusedParameterInspection */
                                   $object_data)
    {
        $decoded_stream_data = '';
    ...
    private function __next_token(/** @noinspection PhpUnusedParameterInspection */
        $page_number, $data, $data_length, $index, $enhanced_statistics = null)
    {
        // Skip spaces
        $count = 0;
    ...
    //
    //		[ sx, rx, ry, tx, ty ]
    private function __matrix_multiply($ma, $mb, /** @noinspection PhpUnusedParameterInspection */
    ...
    private function __next_token_ex(/** @noinspection PhpUnusedParameterInspection */
        $page_number, $data, $data_length, $index, &$token, &$next_index)
    {
        // Skip spaces
        $count = 0;
...
class PdfTexterFontTable extends PdfObjectBase
{
    ...
    public function GetFontAttributes(/** @noinspection PhpUnusedParameterInspection */
        $page_number, $template, $font, &$font_map_width, &$font_mapped)
    {
...
class PdfEncryptionData extends PdfObjectBase
{
    ...
    public function __construct($file_id, $mode, $object_id, $object_data)
    {
        ...
        if ($error_unhandled_version || $error_unhandled_revision) {
            if (PdfToText::$DEBUG)
                error(new DecryptionException ("Unsupported encryption algorithm version {$this -> AlgorithmVersion} revision {$this -> AlgorithmRevision}.",
                    $object_id));
        ...
        if ($this->Decrypter === false) {
            if (PdfToText::$DEBUG)
                warning(new DecryptionException ("Unsupported encryption algorithm #{$this -> EncryptionAlgorithm}, " .
                        ...
                        if ($column_area->Contains($left, $top, $right, $bottom)) {
                            ...
                            if (!isset ($found_lines [$line] [$column_area->Name])) {
                                $found_lines [$line] [$column_area->Name] =
                                    new CapturedColumn ($page, $column_area->Name, $text,
